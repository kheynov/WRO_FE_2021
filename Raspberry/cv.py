...
cap = cv2.VideoCapture('http://192.168.2.153:8000/camera/mjpeg')
...
def find_objects():
    ...
    green_mask_min = (cv2.getTrackbarPos('red_h_min', 'controls'), cv2.getTrackbarPos('red_s_min', 'controls'), cv2.getTrackbarPos('red_v_min', 'controls'))
    green_mask_max = (cv2.getTrackbarPos('red_h_max', 'controls'), cv2.getTrackbarPos('red_s_max', 'controls'), cv2.getTrackbarPos('red_v_max', 'controls'))
    red_mask_min = (0, 0, 0)
    red_mask_max = (25, 255, 255)
    ...
    contours_green, hierarchy_green = cv2.findContours(thresholded_green, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
